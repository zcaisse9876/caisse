...
public:
  ...
  explicit vector(int s);                       // Constructor with specified allocation size
  vector(const T *a, const T *b);               // type deduction for compiler, instantiate with iterator.
  ...
  T &operator[](const int i);                   // Subscript operator for access by index.
  int size() const;                             // Returns the # of items held in the vector.
  int alloc() const;                            // Returns the current # of contiguous blocks of memory allocated to hold type T.
